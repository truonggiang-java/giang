...
public interface ProductRepository extends JpaRepository<Product, Integer> {
	...
	@Query("select new com.example.multikart.dto.response.ProductDetailDtoResponse(p.id,p.name,p.slug,p.discount,p.description) from Product p where p.slug like:slug")
	ProductDetailDtoResponse findBySlug(@Param("slug") String slug);
